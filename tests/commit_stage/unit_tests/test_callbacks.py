...
def test_model_checkpoint():
    ...
    callback.on_epoch_end({"epoch_index": 0, "validation_losses_map": {"total": 100}})

    # Since we are skipping over first epoch, we expect model.save wasn't called
    assert not callback.model.save.called
    callback.on_epoch_end({"epoch_index": 1, "validation_losses_map": {"total": 100}})

    # Reset mock state, call on_epoch_end with larger loss
    callback.model.reset_mock()
    callback.on_epoch_end({"epoch_index": 2, "validation_losses_map": {"total": 200}})
    ...
    callback.model.reset_mock()
    callback.on_epoch_end({"epoch_index": 3, "validation_losses_map": {"total": 50}})
...
def test_early_stopping():
    ...
    callback.on_epoch_end({"validation_losses_map": {"total": 100}})
    ...
    # Loss improved
    callback.on_epoch_end({"validation_losses_map": {"total": 50}})
    assert callback.model.should_continue_training is True
